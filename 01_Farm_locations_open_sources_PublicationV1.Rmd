---
title: "01_Farm_locations_open_sources"
...
#This code used table S10 in the supplementary materials to create the geolocations, in my computer I m=named a CSV with that table in "raw_data/aquaculture_farms_compilation_web.csv" in my project files.
...
```
#save as klm to use in google earth 

```{r}
# Load existing shapefile
aqua_spatial <- st_read(here("output_data", "aquaculture_farms_spatial.shp"))

# Save as KML
st_write(aqua_spatial, 
         here("output_data", "aquaculture_farms_spatial.kml"), 
         driver = "KML",
         delete_dsn = TRUE)
```
#NEXT STEP- Go to Google Earth Pro using the "kml file at output_data/aquaculture_farms_spatial.kml and  create manually polygons for farms on land in NSW, TAS, VIC and
#NEXT STEP 2 (in Rstudio) then open script 2_Insights_Land_based_Leases.Rmd and add datasets you created and others that existed and also add species that now are known because you ran 1_Farm_location_open_sources.Rmd or custodians informed and prepare the land-based datasets for Australian Aquaculture . 
